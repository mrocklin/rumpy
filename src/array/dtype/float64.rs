...
impl Float64Ops {
    #[inline]
    unsafe fn read(ptr: *const u8, byte_offset: isize) -> f64 {
        *(ptr.offset(byte_offset) as *const f64)
    }

    #[inline]
    unsafe fn write(ptr: *mut u8, idx: usize, val: f64) {
...
impl DTypeOps for Float64Ops {
    ...
    unsafe fn write_complex(&self, ptr: *mut u8, idx: usize, real: f64, _imag: f64) -> bool {
        // Store real part only (lossy)
        Self::write(ptr, idx, real);
        true
    }

    unsafe fn read_complex(&self, ptr: *const u8, byte_offset: isize) -> Option<(f64, f64)> {
        Some((Self::read(ptr, byte_offset), 0.0))
    }
}
