//! Int16 dtype implementation.

use super::{BinaryOp, DTypeKind, DTypeOps, ReduceOp, UnaryOp};
use std::cmp::Ordering;

/// Int16 dtype operations.
pub(super) struct Int16Ops;

impl Int16Ops {
    #[inline]
    unsafe fn read(ptr: *const u8, byte_offset: isize) -> i16 {
        *(ptr.offset(byte_offset) as *const i16)
    }

    #[inline]
    unsafe fn write(ptr: *mut u8, idx: usize, val: i16) {
        *(ptr as *mut i16).add(idx) = val;
    }
}

impl DTypeOps for Int16Ops {
    fn kind(&self) -> DTypeKind { DTypeKind::Int16 }
    fn itemsize(&self) -> usize { 2 }
    fn typestr(&self) -> &'static str { "<i2" }
    fn format_char(&self) -> &'static str { "h" }
    fn name(&self) -> &'static str { "int16" }
    ...
    unsafe fn reduce_init(&self, op: ReduceOp, out: *mut u8, idx: usize) {
        let v = match op {
            ReduceOp::Sum => 0,
            ReduceOp::Prod => 1,
            ReduceOp::Max => i16::MIN,
            ReduceOp::Min => i16::MAX,
        };
    ...
    unsafe fn write_complex(&self, ptr: *mut u8, idx: usize, real: f64, _imag: f64) -> bool {
        Self::write(ptr, idx, real as i16);
        true
    }
