...
fn init_default_loops() -> UFuncRegistry {
    ...
    // Complex reduce loops - needed for axis reductions
    macro_rules! register_complex_reduce_loops {
        ($reg:expr, $kind:expr, $T:ty) => {
            // Reduce ops (full-array fallback)
            $reg.register_reduce(
                ReduceOp::Sum,
    ...
    register_complex_reduce_loops!(reg, DTypeKind::Complex128, f64);
    register_complex_reduce_loops!(reg, DTypeKind::Complex64, f32);
